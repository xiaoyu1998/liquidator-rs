...
pub mod mintable_erc20 {
    ...
    impl ::ethers::contract::EthLogDecode for MintableERC20Events {
        fn decode_log(
            ...
            if let Ok(decoded) = BurnFilter::decode_log(log) {
                return Ok(MintableERC20Events::BurnFilter(decoded));
            }
            if let Ok(decoded) = MintFilter::decode_log(log) {
                return Ok(MintableERC20Events::MintFilter(decoded));
            }
    ...
    impl ::core::convert::From<BurnFilter> for MintableERC20Events {
        fn from(value: BurnFilter) -> Self {
            Self::BurnFilter(value)
        }
    }
    impl ::core::convert::From<MintFilter> for MintableERC20Events {
